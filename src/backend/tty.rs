...
pub struct OutputDevice {
    ...
    allocator: Option<GbmAllocator<DrmDeviceFd>>,
...
// Offload clients wake the secondary GPU via the kernel driver directly.
// We only need the primary renderer for dmabuf imports, so dropping the
// secondary reference here is safe.
...
// Query NVIDIA VRAM preservation status. Returns None for non-NVIDIA hardware.
...
impl Tty {
    ...
    fn find_active_primary_for_key(&self, key: DrmNode) -> Option<DrmNode> {
        self.devices.iter().find_map(|(node, device)| {
            if device.render_node == Some(key)
                || (device.render_node.is_none()
                    && (*node == key || self.gpu_power_key(*node) == key))
            {
                Some(*node)
            } else {
                None
...
impl Tty {
    ...
    fn gpu_power_state_mut(&mut self, node: DrmNode) -> &mut GpuPowerState {
        ...
        if !self.gpu_power.contains_key(&key) {
            self.prime_gpu_power_state(node, Some(key));
        }
        self.gpu_power
            .get_mut(&key)
    ...
    pub fn on_prepare_for_sleep(&mut self, niri: &mut Niri, start: bool) {
        if start {
            ...
            // Block new leases during sleep transition.
            self.gpu_gate.set_open_allowed(false, self.pm_generation);

            // Cancel pending timers and async wakeups.
            ...
            // Drain GPU leases off the main loop to keep the compositor responsive.
            if let Err(e) = std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    // Releasing the inhibitor lets logind proceed with suspend.
                    drop(inhibitor);
                })
            {
            ...
            // Reset failed wakeup state so secondary GPUs can retry on resume.
            ...
            // Re-enable leases and restore running state.
    ...
    fn device_added(
        ...
        if !is_retry && node != self.primary_node {
            ...
            if let Some((render_path, expected_dev_id)) = render_target {
                ...
                // Display-only outputs skip slow PCIe wakeups since they lack render nodes.
                // Opening card nodes synchronously is safe here.
                *wakeup_state.lock().unwrap_or_else(|e| e.into_inner()) =
                    WakeupState::WokenNoFd { generation };
            }
        ...
        let (gbm, egl_display, render_node, allocator) = if is_primary {
            let gbm = {
            ...
            (Some(gbm), Some(display), render_node, Some(allocator))
        } else {
            self.remove_pending_device_init(niri, &node);
            debug!("secondary GPU {node:?}: skipping EGL/GBM initialization until output/lease is requested");
            (None, None, None, None)
        };
    ...
    pub(crate) fn ensure_secondary_gpu_initialized(
        &mut self,
        node: DrmNode,
        niri: &mut Niri,
    ) -> anyhow::Result<DrmNode> {
        let device_fd = {
            let device = self.devices.get(&node).context("missing device")?;
            if let Some(render_node) = device.render_node {
                return Ok(render_node);
            }
            device.drm.device_fd().clone()
        };

        debug!("lazy initializing secondary GPU {node:?}");

        let gbm = {
            let _span = tracy_client::span!("GbmDevice::new (lazy)");
        ...
        if egl_device.is_software() {
            bail!("GPU {node:?} is a software renderer, skipping rendering integration");
        }
        ...
        device.gbm = Some(gbm);
        device.egl_display = Some(display);
        device.allocator = Some(allocator);
        device.render_node = Some(render_node);
        ...
        debug!("secondary GPU {node:?} lazy initialized with render node {render_node:?}");
        Ok(render_node)
    }
    ...
    pub(crate) fn check_suspend_device(&mut self, niri: &mut Niri, node: DrmNode) {
        ...
        let token = match niri.event_loop.insert_source(
            ...
            move |_, _, state| {
                ...
                for device in tty.devices.values_mut() {
                    if device.render_node == Some(render_node) {
                        device.egl_display = None;
                        device.allocator = None;
                        device.gbm = None;
                        device.render_node = None;
                    }
                }
    ...
    pub(crate) fn resume_device(
        ...
        let Some(_) = self.devices.get(&node) else {
            return Ok(false);
        };
        ...
        let shared_active = self.devices.iter().any(|(other_node, _other)| {
            *other_node != node
                && self.gpu_power_key(*other_node) == key
                && !self
                    .gpu_power
                    .get(&key)
                    .map(|power| power.suspended)
        ...
        self.ensure_secondary_gpu_initialized(node, niri)
            .map_err(|err| {
                warn!("error initializing secondary GPU {node:?} on resume: {err:?}");
            })?;
    ...
    fn connector_connected(
        ...
        let gbm = device.gbm.clone().context("missing GBM device")?;
        let allocator = device.allocator.clone().context("missing allocator")?;
        ...
        let res = DrmCompositor::new(
            ...
            None,
            allocator.clone(),
            GbmFramebufferExporter::new(gbm.clone(), device.render_node.into()),
            color_formats.clone(),
